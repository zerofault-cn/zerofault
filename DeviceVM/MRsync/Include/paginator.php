...
class Paginator {
	...
	function getRange2() {
		if($this->page==$this->base_page_num + 1) { $this->range2=$this->extra_page_num; }
	...
	function getSecondOf() {
		if($this->page==$this->base_page_num + 1) { $this->second_of=$this->range1 + $this->extra_page_num; }
		else { $this->second_of=$this->range1 + $this->limit; }
		return $this->second_of;
	}
	//method to get the total number of pages.
	function getTotalPages() {
		if($this->extra_page_num) { $this->total_pages = $this->base_page_num + 1; }
		else { $this->total_pages = $this->base_page_num; }
	...
	function getLast() {
		if($this->page == $this->total_pages) { $this->last=0; }
	...
	function getPageName() {
		...
		if($url_query) { $url_query=ereg_replace("(^|&)page=([0-9]{1,})","",$url_query); }
		$this->pagename = $_SERVER["PHP_SELF"]."?".$url_query;
		//$this->pagename = $_SERVER['REQUEST_URI'];
	...
	function getNext() {   
		$this->getTotalPages();
		if($this->total_pages != $this->page) { $this->next = $this->page + 1; }
		return $this->next;
	}
	...
	function getLinkArr() {
		...
		$top_count = $top_range-$this->getCurrent();
		...
		$bottom_count = $this->getCurrent()-$bottom_range;
		
		//echo $bottom_range.", ".$top_range."<br>\n";
		//echo $bottom_count.", ".$limit."<br>\n";
		//echo $this->prev_next."<br>\n";
		
		$bottom_range-=($this->prev_next-$top_count);
		$top_range-=($bottom_count-$this->prev_next);
