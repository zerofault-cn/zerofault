...
//function getSel(tree){
//	var astr = atree.getSelectIds();
//	document.form1.path_str.value=astr;
//}
function onCheck(treeItem,obj){
	selectAllChild(treeItem,obj);
}
function selectAllChild(item,obj)
{
	if(obj.value.slice(-1)=='/')
	{
		...
		{
			var childchild=child[i].childNodes;
		//	alert(childchild.length);
		//	alert(i+child[i].id);
			if(childchild.length==0 && document.getElementById(child[i].id+'-checkbox')==null)
			{
				continue;
			}
			
		//	alert(child[i].id);
		//	alert(document.getElementById(child[i].id+'-cont').style.display);
		//	continue;
			document.getElementById(child[i].id+'-checkbox').checked=obj.checked;
			selectAllChild(child[i],document.getElementById(child[i].id+'-checkbox'));
		}
	}
}
...
var webFXTreeHandler = {
	...
	onCheck   : function (oItem) { onCheck(this.all[oItem.id.replace('-checkbox','')],oItem); },
...
//得到当前的选择字符串
WebFXTree.prototype.getSelectIds = function(selObj) {
	var str = "";
	try{
		var sels = document.getElementsByName(selObj);
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	if(this.radio=="Y"||this.radion=="y"||this.radio=="true"){
		...
		str+="<input type=\"Radio\" id=\"selRadioObj\" name=\"selRadioObj\" value=\""+svalue+"\" onClick=\"SelectDir('"+svalue+"')\" "+selectedstr+" />";
	}else if(this.checkbox=="Y"||this.checkbox=="y"||this.checkbox=="true"){
		if(this.checked=="Y"||this.checked=="y"||this.checked=="true"){
			selectedstr="checked"
		}
		str+="<input type=\"Checkbox\" id=\"" + this.id + "-checkbox\" name=\"selCheckObj\" value=\""+svalue+"\" "+selectedstr+" onClick=\"webFXTreeHandler.onCheck(this);\" />";
	}
...
webFXTreeConfig.emptyErrorTextTemplate = "Error： \"%1%\" TreeNode Not Exist！";
