...
function line_info($id) {
	...
	$result .= '&nbsp;<a href="site_id_'.$row1['term1_sid'].'.html" title="查看经过【'.$term1.'】的所有线路">'.$term1.'</a>('.$row1['term1_start'].'-'.$row1['term1_end'].')';
	$result .= '&nbsp;<a href="site_id_'.$row1['term2_sid'].'.html" title="查看经过【'.$term2.'】的所有线路">'.$term2.'</a>('.$row1['term2_start'].'-'.$row1['term2_end'].')';
	$result .= '&nbsp;票价:'.$row1['fare_norm'].' '.$row1['fare_cond'];
	$result .= '&nbsp;可使用公交卡:'.$row1['ic_card'].'<br />';
	foreach($route_list as $direction=>$route_info)
	{
		...
		foreach($route_info as $id=>$name)
		{
			...
			$result .= '<a href="site_id_'.$id.'.html" title="查看经过【'.$name.'】的所有线路">'.$name.'</a>';
