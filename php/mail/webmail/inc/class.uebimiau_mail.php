...
class UebiMiau extends UebiMiau_core {
	...
	var $_current_folder = "";
	var $_system_folders = Array("INBOX", "Draft", "Sent", "Trash");
	...
	function mail_get_line() {
		...
		if($this->debug) {
			...
			if(eregi("^(\\* )",$buffer) 
				|| eregi("^([A-Za-z0-9]+ (OK|NO|BAD))",$buffer) 
				|| eregi("^(\\+OK|\\-ERR)",$buffer)) {
	...
	function _check_folders() {
		...
		if(!file_exists($this->user_folder)){
			if(!@mkdir($this->user_folder,0777)) 
		...
		if($this->mail_protocol == "imap") {
			...
			for($i = 0; $i < count($boxes); $i++) {
				...
				if(!in_array($current_folder, $this->_system_folders)) {
		...
		$system_folders = $this->_system_folders;
	...
	function mail_delete_msg($msg, $send_to_trash = 1, $save_only_read = 0) {
		...
		if($this->mail_protocol == "imap") {
			...
			if( $send_to_trash && 
				...
				//modify mail size
				$lFileSize = 0;
				$lSplitPos = strrpos($msg["localname"], ",S=");

				if ($lSplitPos !== false)
				{
					$strTemp = substr($msg["localname"], $lSplitPos+3);
					$lSplitPos = strrpos($strTemp, ".");
					if ($lSplitPos !== false)
						$lFileSize = intval(substr($strTemp, 0, $lSplitPos));
					else
						$lFileSize = intval($strTemp);
				}
				else
					$lFileSize = filesize($msg["localname"]);
				
				if (file_exists($this->user_folder.'mailsize.dat') == true
					&& filesize($this->user_folder.'mailsize.dat') != 0){

					$strLine = '-'.$lFileSize." -1\r\n";
					$fp = fopen($this->user_folder.'mailsize.dat', "a");
					fwrite($fp, $strLine);
					fclose($fp);
		...
		for($i = 0; $i < count($msglist); $i++) {
			...
			*/
			if($this->mail_protocol != "imap") {
				...
				$msglist[$i]["flags"] = $mail_info["x-um-flags"]; //strtoupper($headers["x-um-flags"]);
			}
	...
	function _get_local_name($message,$boxname, $size = 0) {
		$now = time();
		
		srand($now);
		$randval = rand(1, 10000);
		$threadid = rand(256, 8196);
		
		if ($size == 0)
			$size = 2048;

		$flocalname = trim($this->user_folder."$boxname/".$now.'.'.$randval.'.'.$threadid.',S='.$size);
	...
	function mail_list_boxes($boxname = "*") {
		...
		if($this->mail_protocol == "imap") {
			...
			while($entry=$d->read()) {
				if(in_array($entry,$this->_system_folders)) 
					$entry = $entry;
	...
	function mail_create_box($boxname) {
		...
		if($this->mail_protocol == "imap") {
			...
			if(@mkdir($this->user_folder.$boxname,0777)) 
				return 1;
			else 
				return 0;
		}
	...
	function mail_delete_box($boxname) {
		if($this->mail_protocol == "imap") {
			...
			if(is_dir($this->user_folder.$boxname)) {
				//modify mail size
				$dirpath = $this->user_folder.$boxname;
				if (substr($dirpath, -1) <> "/") 
					$dirpath = $dirpath."/";
				...
				while ($file = readdir($all)) { 
					if (!is_dir($dirpath.$file) 
						...
						$lFileSize = 0;
						$lSplitPos = strrpos($file, ",S=");

						if ($lSplitPos !== false)
						{
							$strTemp = substr($file, $lSplitPos+3);
							$lSplitPos = strrpos($strTemp, ".");
							if ($lSplitPos !== false)
								$lFileSize = intval(substr($strTemp, 0, $lSplitPos));
							else
							...
							$lFileSize = filesize($dirpath.$file);
	...
	function mail_save_message($boxname,$message,$flags = "") {
		...
		if(is_dir($this->user_folder.$boxname)) {
			$mailsize = strlen($message);
			
			$email = $this->fetch_structure($message);
			$mail_info = $this->get_mail_info($email["header"]);
			$filename = $this->_get_local_name($mail_info, $boxname, $mailsize);
			...
			//modify mail size	
			$strLine = '';
			if (file_exists($this->user_folder.'mailsize.dat') == false 
				|| filesize($this->user_folder.'mailsize.dat') == 0){
				$strLine .= "0S 0C\r\n";
			}
	...
	function mail_set_flag(&$msg,$flagname,$flagtype = "+") {
		...
		if(file_exists($msg["localname"])) {
			...
			$arrFileName = explode(',S=', $msg["localname"]);
			if (!empty($arrFileName[1])){
				$arrFlag = explode('.', $arrFileName[1]);
			}
			
			if (empty($arrFlag[0]))
				$arrFlag[0] = filesize($msg["localname"]);
			...
			if($flagtype == "+") {
				switch (strtoupper($flagname)){
					case "\\DELETED":
						$bfind = false;
						$newflag = '';
						for ($i = 0; $i < strlen($oldflag); $i++){
							$ch = strtoupper($oldflag[$i]);
							if ($ch != 'U')
								$newflag .= $oldflag[$i];
							if ($ch == 'D')
								$bfind = true;
				...
				$arrFileName[1] = implode('.', $arrFlag);
				$newmsgfile = $arrFileName[0].',S='.$arrFileName[1];
				if (rename($msg["localname"], $newmsgfile))
					$msg["localname"] = $newmsgfile;
			}
