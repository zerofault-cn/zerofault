...
*/
		...
		$fd = fopen($strfile,"rb");
		$result = fread($fd, filesize($strfile));		
		fclose($fd);
...
//    		$string = "=?".$this->charset."?Q?".str_replace("+","_",str_replace("%","=",urlencode($string)))."?=";
	...
	function add_body($strbody) {
		if(!$this->allow_scripts) 
			$strbody = $this->filter_scripts($strbody);
	...
	function get_names($strmail) {
		...
		for($i = 0; $i < strlen($strmail); $i++) {
			...
			if(strpos($chartosplit,$thischar) !== false 
				&& !$inthechar && $closed) {
				$armail[] = $temp; 
				$temp = "";
			} 
	...
	function build_alternative_body($ctype,$body) {
		...
		foreach($parts as $index => $value) {
			...
			else if (!$this->use_html && $ctype == "text/plain") {
				$part = $parts[$index];
				break;
			}
		}
		...
		if(!$this->use_html)
			$body = $this->build_text_body($body);
	...
	function build_complex_body($ctype,$body) {
		...
		for($i = 0;$i < count($part); $i++) {
			...
			elseif($is_download) {
				...
				$filename 		= $thisattach["filename"];//$this->temp_folder."_attachments/".$this->mail_user.'_'.md5($thisattach["boundary"])."_".$thisattach["name"];
				...
				elseif($thisattach["content-type"] == "message/rfc822") {
					$temp_attach	= true;
				}
	...
	function build_text_body($body) {
		...
		return "<font style=\"font-family: Arial, Helvetica, sans-serif; font-size: 10.8pt;\">$body</font>";
	}
	...
	function build_attach($header, $body, $boundary, $part, $mode="info", $down=0, $nametosave="tmp.eml") {
		...
		if($filename == "" && $Atype[0] == "message") {
			...
			unset($attachheader);
			$filename = trim(substr(ereg_replace("[^A-Za-z0-9]","_",$filename),0,20));
		...
		case "info":
			...
			$temp_array["filename"] = $this->temp_folder."_attachments/".$this->mail_user.'_'.md5($boundary)."_".$filename;;
			...
			break;
		case "down":
			$content_type = ($down)? "application/RFC822" : strtolower($ctype2);
	...
	function compile_body($body,$enctype,$ctype) {
		...
		elseif(strtolower($enctype) == "quoted-printable")
			//$body = $this->decode_qp($body);			//decode error
	...
	function get_mail_info($header) {
		...
		$myarray["x-um-flags"] = $headers["x-um-flags"];
