...
function onLoad()
{
	...
	{
		map.setCenter(new GLatLng(parseFloat(K_GetQueryString("center.y")),parseFloat(K_GetQueryString("center.x"))),zoom,G_HYBRID_MAP);
		if(K_GetQueryString("pInfo")!="")
		{
			CreateMarker1(new GLatLng(parseFloat(K_GetQueryString("center.y")),parseFloat(K_GetQueryString("center.x"))),icon_Blue,K_GetQueryString("pInfo"));
		}
		else
		{
			CreateMarker1(new GLatLng(parseFloat(K_GetQueryString("center.y")),parseFloat(K_GetQueryString("center.x"))),icon_Blue);
		}
	}
	else
	{
	//	map.setCenter(new GLatLng(36.94,106.08),zoom,G_NORMAL_MAP);
		map.setCenter(new GLatLng(22.56053,114.03145),16,G_HYBRID_MAP);
	}
	...
	request.onreadystatechange = function()
	{
		if (request.readyState == 4)
		{
			if (request.status == 200)
			{
				...
				point1=new GLatLng(lat1,lng1);
				map.panTo(point1);

				if(lng0==200 && lat0==100)//初始位置
				{
					map.setCenter(point1,16,G_HYBRID_MAP);//将起点位置居中,并放大地图到合适等级,设置合适地图类型
					CreateMarker1(point1,icon_Red,'起点<br>'+date+'<br>'+time);
				}
				else if(lat0!=lat1 || lng0!=lng1)//周期时间内有移动,先创建折线,再添加汽车标识
				{
					polyline = new GPolyline([new GLatLng(lat0,lng0),new GLatLng(lat1,lng1)], "#0000ff", 3,1);
