...
//将标注显示在K_Tree之中的对象
	function K_OverlayTree(map,treeNode,handle)
	{
		...
		this.defaultStyle.IconStyle.Icon.href="/map/Icons/palette-3.png";
	...
	K_OverlayTree.prototype.ParseKmlPlace=function(placeMark,kmlStyle,parentNode)
	{
		...
		if(placeMark.selectSingleNode("Point"))
		{
			...
			if(p.length<2)
				return;
			point=new GPoint(parseFloat(p[0]),parseFloat(p[1]));
		...
		if(placeMark.selectSingleNode("LineString"))
		{
			...
			{
				ps=p[j].split(",");
				p[j]=new GPoint(parseFloat(ps[0]),parseFloat(ps[1]));
			}
