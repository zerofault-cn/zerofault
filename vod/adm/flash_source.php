...
<caption><span style="color:#3399cc">Flash资源列表</span> 选择分类</caption>
<tr bgcolor=white>
	<?
...
<tr bgcolor='<?=$bgcolor?>'>
	...
	{
		?>
		<a style='color:blue' title='文件路径:<?=$flashfile_path?>'>有</a>
		<?
	}
	else
	{
		?>
		<a style='color:red' title='文件路径:<?=$flashfile_path?>'>无</a>
		<?
	}
	?>
	<td align=center><input type=button onclick="window.open('flash_modify_source_1.php?id=<?=$id?>','','width=450,height=280,toolbar=no,status=no,scrollbars=auto,resizable=yes');" value="修改"><input type=button onclick='del(<?=$id?>)' value="删除"></td>
</tr>
<?
}
?>
...
if($offset!=0)
{
	...
	?>
	<a href="?content=flash_source&type_id=<?=$type_id?>&offset=0">【最前】</a>&nbsp;&nbsp;
	<a href="?content=flash_source&type_id=<?=$type_id?>&offset=<?=$preoffset?>">【前一页】</a>&nbsp;&nbsp;
	<?
}
...
if(($offset+$pageitem)<$rowCount)
{
	...
	?>
	<a href="?content=flash_source&type_id=<?=$type_id?>&offset=<?=$nextoffset?>">【后一页】</a>&nbsp;&nbsp;
	<a href="?content=flash_source&type_id=<?=$type_id?>&offset=<?=$endpage?>">【最后】</a>&nbsp;&nbsp;
	<?
}
?>
...
<tr bgcolor=white>
	<td colspan=6 align=center><a href='index.php?content=flash_add_source_1' style="color:red">【添加内容】</a></td>
</tr>
