...
ResultSet rs = opendb.executeQuery("select * from user_info where user_account='" + account + "'");
if(rs != null && rs.next())
{
%>
<html>
...
function img_click() {	
  location="login_3.jsp?password=" + password1+"&account=<%=account%>";
//	location="dl0.jsp?password=" + document.form1.password.value;
	}

var shift=0;
function img_click_2(m,n) {
	if(shift==0) password1 += m;
	else password1 += n;
		var p2 = '';
		for(i=0;i<password1.length;i++) p2 += '*'
	t1 = document.getElementById("password");
	t1.innerHTML = password0 + p2 + password2;
	if(password1.length==20)
	{ 
		password1='';
		t1.innerHTML = password1;
	}
}
...
<!--
body {
	background-image: url(file:///usr/suit/newebox/image/keyboard/login.jpg);
}
-->
</style></head>

<body leftMargin=0 topMargin=0 bgcolor="#0a56d">
...
<%
	...
	response.sendRedirect("error.jsp?wrong=account");
%>