...
class BoardAction extends BaseAction{
	...
	public function index(){
		...
		foreach ($rs as $val) {
			if ('Board' == $val['type']) {
				$result[md5($val['description'])] = $this->dao->relation(true)->where(array('type'=>'Board', 'description'=>$val['description']))->order('Internal_PN')->select();
			}
			else {
				$result[md5($val['description'])] = $this->dao->relation(true)->where('id='.$val['id'])->select();
			}
		}
	...
	public function submit() {
		...
		if($file['size']>0) {
			...
			$config=array(
				...
				'savePath'=>'Attach/Product/',
				'saveRule'=>'time',
			);
			$upload = new UploadFile($config);
			if(!$upload->upload()){
				self::_error($upload->getErrorMsg());
			}
			else {
				$info =  $upload->getUploadFileInfo();
				$this->dao->attachment = $info[0]['savepath'].$info[0]['savename'];
			}
			/*
	...
	public function import() {
		...
		$msg  = 'Board Basic Data importing result:<br />';
