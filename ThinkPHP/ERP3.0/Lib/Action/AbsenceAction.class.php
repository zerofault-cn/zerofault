...
//		$this->time = mktime(0,0,0,2,28,2012);
	...
	public function form() {
		...
		$this->display(ACTION_NAME.(empty($type)?'':'-'.$type));
	}
	...
	public function manage() {
		...
		elseif('' != $tmp=session(ACTION_NAME.'_staff_id')) {
			$staff_id = $tmp;
		}
		else {
			$staff_id = 0;
		}
		session(ACTION_NAME.'_staff_id', $staff_id);
		$this->assign('staff_opts', self::genOptions(M('Staff')->where(array('status'=>1))->select(), $staff_id, 'realname'));

		if (isset($_REQUEST['type'])) {
			$type = $_REQUEST['type'];
		}
		elseif('' != $tmp=session(ACTION_NAME.'_type')) {
			$type = $tmp;
		}
		else {
			$type = '';
		}
		session(ACTION_NAME.'_type', $type);
		global $LeaveType;
		$type_arr = array();
		...
		elseif(''!= $tmp=session(ACTION_NAME.'_from')) {
			$from = session(ACTION_NAME.'_from');
		}
		else{
			$from = date('Y-m-d', $this->time-15*86400);
		}
		session(ACTION_NAME.'_from', $from);
		$this->assign('from', $from);

		if(isset($_REQUEST['to'])) {
			$to = $_REQUEST['to'];
		}
		elseif(''!= $tmp=session(ACTION_NAME.'_to')) {
			$to = session(ACTION_NAME.'_to');
		}
		else{
			$to = date('Y-m-d', $this->time+15*86400);
		}
		session(ACTION_NAME.'_to', $to);
		$this->assign('to', $to);

		if(isset($_REQUEST['status'])) {
			$status = $_REQUEST['status'];
		}
		elseif(''!= $tmp=session(ACTION_NAME.'_status')) {
			$status = session(ACTION_NAME.'_status');
		}
		else{
			$status = '-';
		}
		session(ACTION_NAME.'_status', $status);
		$status_arr = array(
		...
		session('sub', MODULE_NAME.'/'.ACTION_NAME);
		$this->display();
	}
