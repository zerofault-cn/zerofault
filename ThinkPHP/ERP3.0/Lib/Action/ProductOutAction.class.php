...
class ProductOutAction extends BaseAction{
	...
	public function _initialize() {
		global $MODULE;
		if (empty($MODULE)) {
			session('top', 'Inventory Output Management');
		}
		//else $MODULE='Asset'
	...
	private function index($action='apply', $fixed='') {
		...
		if (!empty($location_id)) {
			session('sub', MODULE_NAME.'/'.ACTION_NAME.'/id/'.$location_id);
		}
		else {
		...
		elseif(''!=(session(ACTION_NAME.'_status'))) {
		...
		session(MODULE_NAME.'_'.ACTION_NAME.'_limit', $limit);
	...
	public function form() {
		...
		if ($id>0) {
			...
			if (!empty($_REQUEST['lp_id'])) {//act from asset list
				...
				if('transfer'==$action) {
					session('sub', MODULE_NAME.'/transferOut');
				}
				elseif('return'==$action) {
					session('sub', MODULE_NAME.'/returns');
				}
				$code = ++ $max_code;
			}
			if('transfer'==$action) {
