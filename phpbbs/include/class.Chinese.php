...
class Chinese
{
	 ...
	 */
	var $config  =  array(
		'codetable_dir'         => "/phpbbs/include/codetable/",           //  存放各种语言互换表的目录
		'SourceLang'            => '',                    //  字符的原编码
		'TargetLang'            => '',                    //  转换后的编码
	...
	function OpenTable()
	{
		...
		if ($this->config['SourceLang']=="GB2312") {
			...
			if ($this->config['TargetLang'] == "BIG5") {
				...
				if (is_null($this->ctf)) {
					echo "打开转换表文件".$this->config['codetable_dir'].$this->config['GBtoBIG5_table']."失败！";
					exit;
				}
			}
			...
			if ($this->config['TargetLang'] == "PinYin") {
				...
				if (!$tmp) {
					echo "打开转换表文件".$this->config['codetable_dir'].$this->config['GBtoPinYin_table']."失败！";
					exit;
				}
				//
				$i = 0;
			...
			if ($this->config['TargetLang'] == "UTF8") {
				...
				if (!$tmp) {
					echo "打开转换表文件".$this->config['codetable_dir'].$this->config['GBtoUnicode_table']."失败！";
					exit;
				}
		...
		if ($this->config['SourceLang']=="BIG5") {
			...
			if ($this->config['TargetLang'] == "GB2312") {
				...
				if (is_null($this->ctf)) {
					echo "打开转换表文件".$this->config['codetable_dir'].$this->config['BIG5toGB_table']."失败！";
					exit;
				}
			}
			// 假如转换目标编码为 UTF8 的话
			if ($this->config['TargetLang'] == "UTF8") {
				...
				if (!$tmp) {
					echo "打开转换表文件".$this->config['codetable_dir'].$this->config['BIG5toUnicode_table']."失败！";
					exit;
				}
				$this->unicode_table = array();
				while(list($key,$value)=each($tmp))
