...
elseif (submitcheck('ajaxupload')) {
	...
	//检查并创建相册
	$rs = $_SGLOBAL['db']->query("SELECT albumid FROM ".tname('album')." WHERE uid=".$_SGLOBAL['supe_uid']." and albumname='".$albumname."' LIMIT 1");
	if ($_SGLOBAL['db']->num_rows($rs)>0) {
		$albumid = $_SGLOBAL['db']->result($rs, 0);
	}
	else {
		$setarr = array();
		...
		$setarr['username'] = $_SGLOBAL['supe_username'];
		$setarr['dateline'] = $setarr['updatetime'] = $_SGLOBAL['timestamp'];
		$setarr['friend'] = $friend;
		$setarr['password'] = $password;
		$setarr['target_ids'] = $target_ids;

		$albumid = inserttable('album', $setarr, 1);
	}
	$uploadfiles = pic_save($_FILES['filename'], $albumid, $_FILES['filename']['name'], 0);
	$html = '<script>';
	if ($uploadfiles['picid']>0) {
		$html .= 'parent.successUpload('.$uploadfiles['picid'].', "'.$uploadfiles['filename'].'", "'.$uploadfiles['filepath'].($uploadfiles['thumb']?'.thumb.jpg':'').'");';
	}
	else {
		$html .= 'parent.failUpload();';
	}
	$html .= '</script>';
	die($html);
}
