...
if($op == 'add') {
	...
	if(!empty($_GET['searchsubmit']) || !empty($_GET['searchmode'])) {
		...
		$carnumber_prefix = array('京','津','沪','渝','冀','豫','云','辽','黑','湘','皖','鲁','新','苏','浙','赣','鄂','桂','甘','晋','蒙','陕','吉','闽','贵','粤','青','藏','川','宁','琼');
		if (in_array(mb_substr($searchkey, 0, 1), $carnumber_prefix)) {
		...
		if($wherearr) {
			...
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				...
				if (!empty($value['car_model'])) {
					$car_model = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT name FROM ".tname('carmodel')." WHERE id=".$value['car_model']), 0);
				}
				if (!empty($value['car_profile'])) {
					$car_profile = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT name FROM ".tname('carmodel')." WHERE id=".$value['car_profile']), 0);
				}
				...
				if (!empty($value['city_id'])) {
					$value['region_str'] .= '-'.$_SGLOBAL['db']->result($_SGLOBAL['db']->query("select name from ".tname('region')." where id=".$value['city_id']), 0).'市';
				}
				if (!empty($value['region_id'])) {
					$value['region_str'] .= '-'.$_SGLOBAL['db']->result($_SGLOBAL['db']->query("select name from ".tname('region')." where id=".$value['region_id']), 0);
				}
