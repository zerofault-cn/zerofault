...
$sql1="select user_id,addvote from ".$sms_table." where status=1 and polltime>=".$date." and polltime<".($date+24*60*60);
...
{
	$arr[intval($row['user_id'])]+=$row['addvote'];
}
arsort($arr);
$i=0;
while(list($key,$val)=each($arr))
{
	...
	$tmpval=min(4200,$val);
	echo '<div style="border-bottom:1px dotted #aaa;"><span style="width:510px;float:right;text-align:left;padding:2px;"><span style="width:'.($tmpval/10).'px;background-color:#00f;margin:2px;"> </span>'.$val.' 票</span><span style="width:60px;float:left;text-align:right;padding:2px;"><a href="smsvote_info.php?mm_id='.$key.'&date='.$date.'" target="_blank">'.sprintf("%04d",$key).'</a>：</span></div>';
}
echo '</div>';

$sql2="select user_id,count(*) as count from ".$ip_table." where polltime>=".$date." and polltime<".($date+24*60*60)." group by user_id order by count desc";
...
while($row=$db->sql_fetchrow($result2))
{
	...
	echo '<div style="border-bottom:1px dotted #aaa;"><span style="width:510px;float:right;text-align:left;padding:2px;"><span style="width:'.($tmp_count/10).'px;background-color:#00f;margin:2px;"> </span>'.$count.' 票</span><span style="width:60px;float:left;text-align:right;padding:2px;"><a href="ipvote_cheat.php?mm_id='.$user_id.'&date='.$date.'" target="_blank">'.sprintf("%04d",$user_id).'</a>：</span></div>';
}
echo '</div>';
...
$sql3="select max(id) from user_info where pass=1";
...
$sql4="select user_id,count(*) as count from user_comment where addtime>=".$date." and addtime<".($date+24*60*60)." group by user_id order by count desc limit 10";
echo '<div style="width:580px;margin-top:20px;border:1px solid #00f;text-align:left;padding-top:4px;">';
echo '当天留言增加数排行:';
...
while($row=$db->sql_fetchrow($result4))
{
	...
	echo '<div style="border-bottom:1px dotted #aaa;"><span style="width:510px;float:right;text-align:left;padding:2px;"><span style="width:'.($tmp_count/3).'px;background-color:#00f;margin:2px;"> </span>'.$count.' 条</span><span style="width:60px;float:left;text-align:right;padding:2px;"><a href="../comment.php?id='.$user_id.'" target="_blank">'.sprintf("%04d",$user_id).'</a>：</span></div>';
}
echo '</div>';
