...
class FlashListAction extends Action {
	/**
    * 
    * @access public
    * @param array &$request
    ...
    */
    function execute(&$actionMap,&$actionError,$request,&$response,$form){
		...
		$sql_all = "select count(*) as num from flash_images"; // wenzhang zongshu
		...
		$url = 'main.php?do=flash_list&channel_name=' . $channel_name . '&p='.$request['p'];
		...
		if($itemSum>0)
		{
			$sql = "select * from flash_images order by id desc limit ".($request['p']-1)*$pageSize.", ".$pageSize;
