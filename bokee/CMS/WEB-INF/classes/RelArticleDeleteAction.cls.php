...
class RelArticleDeleteAction extends Action {
    ...
    */
    function execute(&$actionMap,&$actionError,$request,&$response,$form){
        ...
        if($id>0)
        {
       		$sql0="select article_id,source from rel_article_subject where id=".$id;
			$row=$dao->GetRow($sql0);
			$article_id=$row[0];
			$source=$row[1];
			if($source=='cms')
			{
				$sql1="delete from article where id=".$article_id;
			}
			else
			{
				$sql1="delete from rss_entry_attach where id=".$article_id;
			}
			$dao->Query($sql1);
       		$sql2="delete from rel_article_subject where id=".$id;
			if($dao->Query($sql2))
       		{
