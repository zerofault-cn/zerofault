...
class ChannelListAction extends Action {
    ...
    */
    function execute(&$actionMap,&$actionError,$request,&$response,$form){
		...
		$sql_all = "select count(*) as num from channel "; // wenzhang zongshu
		...
		$url = 'main.php?do=channel_list&p='.$request['p'];
		...
		$sql = "select * from channel order by id desc limit ".($request['p']-1)*$pageSize.", ".$pageSize;
		$rows = $dao->GetPlan($sql);
		$rows_num = count($rows);
		
		$response['channel'] = $rows;
