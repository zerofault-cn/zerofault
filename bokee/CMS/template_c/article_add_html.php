...
<table width="100%" border="0" cellpadding="2" cellspacing="2" bgcolor="#eeeeee">
  <tr>
    <td nowrap>文章标题</td>
    <td colspan="2"><input name="title" class="title" type="text" id="title" size="50" maxlength="100" value="<?php
echo $_obj['title'];
?>
...
?>
  ...
  <tr>
    <td nowrap>发布时间</td>
    <td colspan="2"><input name="plush_time" class="title" type="text" id="title" size="5" maxlength="2" value="<?php
echo $_obj['plush_time'];
?>
">小时以后发布</td>
  </tr>
  <tr>
    <td nowrap>副标题</td>
    <td colspan="2"><input name="sub_title" type="text" id="sub_title" size="50" maxlength="25" value="<?php
echo $_obj['sub_title'];
?>
"></td>
  </tr>
...
?>
  ...
  <tr>
    <td nowrap>组图描述</td>
    <td colspan="2"><textarea name="group_image_desc" cols="73" rows="10" id="grout_image_desc"><?php
echo $_obj['group_image_desc'];
?>
</textarea>
    <div class="error"><?php
echo $_obj['action_error_group_image_desc'];
?>
</div></td>
  </tr>
  <tr>
...
?>
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <input type="button" name="add_to_other_subjects" value="添加到其他栏目" onClick="javascrpt:window.open('main.php?do=subject_select&channel_name=<?php
echo $_obj['channel_name'];
?>
', 'select_channels', 'height=600, width=600, top=0,left=0, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no') ">
...
?>
   ...
   <tr>
	...
	<?php
if (!empty($_obj['jump'])){
if (!is_array($_obj['jump']))
$_obj['jump']=array(array('jump'=>$_obj['jump']));
$_tmp_arr_keys=array_keys($_obj['jump']);
if ($_tmp_arr_keys[0]!='0')
$_obj['jump']=array(0=>$_obj['jump']);
$_stack[$_stack_cnt++]=$_obj;
foreach ($_obj['jump'] as $rowcnt=>$jump) {
$jump['ROWCNT']=$rowcnt;
$jump['ALTROW']=$rowcnt%2;
$jump['ROWBIT']=$rowcnt%2;
$_obj=&$jump;
?>
	 <input name="radio_is_jump" type="radio" value="<?php
echo $_obj['value'];
?>
" <?php
echo $_obj['checked'];
?>
 onclick=javascript:passAlter();><?php
echo $_obj['name'];
?>
...
?>
    ...
    <input type="text" name="text_jump_url" size="50"> <div class="error"><?php
echo $_obj['action_jump_url'];
?>
</div></td>
  </tr>
