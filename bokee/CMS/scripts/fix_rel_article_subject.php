...
//	$db = "cms_" . $rows_channel[$i]['dir_name'];
	...
	*/
	...
	for($j=0;$j<$rows_num_feed;$j++)
	{
		...
		{
			$sql_entry = "select * from entry where feed_id=" . $rows_feed[$j]['feed_id'] . " and id>$mid_local order by id";
...
//			echo "new entry num: " . $rows_num_entry . "\n";
			...
			{
				//判断是否数据库中已经存在
			//modified by zhuming at 2007/3/13
			//	$sql_entry_exists = "select count(*) as num from rss_entry_attach where entry_id=" . $rows_entry[$k]['id'];
			/*	$sql_entry_exists = "select id from rss_entry_attach where url='" . $rows_entry[$k]['url']."'";
				$num = $dao->GetOne($sql_entry_exists);
				if($num>0)
				{
					//如果已经存在则跳过，继续下一�?
					continue;
			...
			*/	
				...
				//根据表rss_feed_attach来判断来�?
				//$source = $rows_feed[$j]['source'];
				$source = "rss";
			...
			//	if($rows_channel[$i]['dir_name'] == 'blog')
				...
				{
				//	$dnsName = $rows_channel[$i]['dir_name'];
