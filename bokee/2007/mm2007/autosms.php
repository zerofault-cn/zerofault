...
//检查是否超过每月投票限制
$sql2="select count(*) from ".$sms_table." where polltime>".$month_start." polltime<".($month_start+28*86400)." and (status=0 || status=1) and addvote=1 and feephone='".$feephone."'";
$sql20="select 5*count(*) from ".$sms_table." where polltime>".$month_start." polltime<".($month_start+28*86400)." and (status=0 || status=1) and addvote=5 and feephone='".$feephone."'";
...
$sql4="update mm_info set smsvote=smsvote+5,allvote=allvote+150 where id=".$mm_id;
