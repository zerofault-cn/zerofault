...
function check()
{
	...
	for(i=0;i<fields.length;i++)
	{
		...
		if(ckValue!=null)
		{
			...
			if(ckValue=='所在位置' && fields[i].value=='-')
			{
				alert('请选择您所在的位置');
				document.form1.input_location1.focus();
				return false;
			}
			if(fields[i].value=='')
			{
...
{
	xmlhttp.open("get","mm_friend/signup.php?action=checkUrl&url="+url+"&"+Math.random(),false);
	xmlhttp.send(null);
