...
{
	$sql="select comment.*,user_info.realname,user_info.blogurl from comment,user_info where comment.user_id=user_info.id order by comment.id desc";
}
//echo $sql;
//$result=$db->sql_query($sql);
$total=100;//$db->sql_numrows($result);
...
while($row=$db->sql_fetchrow($result))
{
	...
	{
		$t_realname=getField($user_id,'realname');
	...
	$tpl->assign_block_vars("list", array(
		...
		"REALNAME" => (''!=$t_blogname)?$t_blogname:((strlen($user_id)==1&&$user_id==0)?'所有人':$row['realname']),
